...
from LowFrequency.LowFrequency import LowFrequency
from LowFrequency.DataBase import DataBase
from LowFrequency.LossFunction import LossFunction
from LowFrequency.Vizualise import plotState
...
def Train(training_config:TrainingConfig, error_config:TrainingLoss, outFile:str, runName:str, database:DataBase, database_validation:DataBase, visual:bool = False):
    ...
    for ep in range(training_config.epochs):
        ...
        for i, data in enumerate(database_loader):
            ...
            # split the data
            descriptor_1, descriptor_2, latent_last, latent_goal, pose_goal = data

            if torch.cuda.is_available():
                descriptor_1 = descriptor_1.cuda()
                descriptor_2 = descriptor_2.cuda()
            ...
            out_latent = model.forward(descriptor_1, descriptor_2, latent_last)[0]
        ...
        with torch.no_grad():
            ...
            writer.add_scalar("LF_Training/Total_Weighted", losses_sum_weighted_training, ep)
            writer.add_scalar("LF_Training/Total_Unweighted", sum(losses_training), ep)
            for i in range(len(labels)):
                writer.add_scalar("LF_Training/"+labels[i], losses_training[i], ep)
            writer.flush()
        ...
        with torch.no_grad():
            ...
            # Add to tensorboard
            labels = error_config.getNames()
            writer.add_scalar("LF_Validation/Total_Weighted", losses_sum_weighted_validation, ep)
            writer.add_scalar("LF_Validation/Total_Unweighted", sum(losses_validation), ep)
            for i in range(len(labels)):
                writer.add_scalar("LF_Validation/"+labels[i], losses_validation[i], ep)
            writer.flush()
