...
class DataBase(Dataset):
    ...
    def __init__(self, source_dir:str, vae_dir:str, sequenceLengthLong:int = 30, sampling_down_factor:float=0.1):
        self.fileNames = []
        loadedFiles = []
        self.fileLengths = []
    ...
    def __getitem__(self, idx:int) -> Tuple[torch.Tensor]:
        ...
        cumsum = torch.cumsum(self.fileLengths-self.deltaT * 2, 0)
        file_idx = torch.where(cumsum > idx)[0][0].item()
        frame = idx + (2 * self.deltaT) * file_idx
        ...
        for i in range(0, self.deltaT * self.sequenceLengthLong):
            ...
            latents.append(self.latents[frame + i])
        
        features = torch.vstack(features)
        true_poses = torch.vstack(true_poses)
        latents = torch.vstack(latents)

        return features, latents, true_poses