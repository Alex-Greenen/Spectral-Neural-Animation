...
def LossFunction(latentRecon:torch.Tensor, latentTrue:torch.Tensor, true_data:torch.Tensor, genLoss: torch.Tensor, database:DataBase, frames_per_ep:int) -> TrainingLoss:
    ...
    with torch.no_grad(): 
        R_r, Root_Height_r, Root_Position_Velocity_r, Root_Rotation_xz_r, Root_RotationVel_y_r, contacts_r = torch.split(AE_network.decoder(latentRecon), database.poseDimDiv, dim=-1)
        R_t, Root_Height_t, Root_Position_Velocity_t, Root_Rotation_xz_t, Root_RotationVel_y_t, contacts_t = torch.split(true_data, database.poseDimDiv, dim=-1)
    ...
    return TrainingLoss(HighFrequencyLossType, [latent, genLoss, angular, fk, posVel, RotXZ, RotVelY, height, feet, contacts, DirectReconSmooth, FkReconSmooth, PosVelSmooth, RotXZSmooth, RotVelYSmooth, HeightSmooth, FeetSmooth])