...
def extract_feet_contacts(pos, lfoot_idx, rfoot_idx, velfactor=0.02):
    ...
    contacts_l = (np.sum(np.sum(lfoot_xyz, axis=-1),axis=-1, keepdims=True) < velfactor)

    rfoot_xyz = (pos[1:, rfoot_idx, :] - pos[:-1, rfoot_idx, :]) ** 2
    contacts_r = (np.sum(np.sum(rfoot_xyz, axis=-1),axis=-1, keepdims=True) < velfactor)

    # Duplicate the last frame for shape consistency
    contacts_l= np.vstack((contacts_l, contacts_l[-2:-1,:]))
    contacts_r= np.vstack((contacts_r, contacts_r[-2:-1,:]))
    #contacts_r = np.concatenate([contacts_r, contacts_r[-1:]], axis=0)[:,0]
